...
void speck_encrypt(uint16_t const pt[2], uint16_t ct[2], uint16_t const K[ROUNDS]) {
    ct[0] = pt[0];
    ct[1] = pt[1];
...
void speck_decrypt(uint16_t const ct[2], uint16_t pt[2], uint16_t const K[ROUNDS]) {
    pt[0] = ct[0];
    pt[1] = ct[1];
    for (int i = 0; i < ROUNDS; i++) {
...
int main(void) {
    ...
    printf("==> Plain text: %u %u (decrypted cipher text)\n\n", buffer[0], buffer[1]);
