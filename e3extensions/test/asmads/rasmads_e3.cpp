#include <iostream>
// #include "../../src/e3extensions/secureint.h"
#include "../../src/e3extensions/bigint.h"

#define KEYSIZE 2048
#define ENCSIZE (2*KEYSIZE)
// #define LEN 1+13*32

using namespace std;

// Driver program to test above functions
int main()
{
#if KEYSIZE == 256
	/* key 256 */
	Unumber pri("e5f7bb45339f08ac97bfdf1e4ae08cff89467a2de492fabc6dd6e403cf9ea500a93f159a1da147f6303d4349958c25d8f78a7b1416fa6e86c25c317f2470a51", 16);
	Unumber pub("e5f7bb45339f08ac97bfdf1e4ae08d0c71bb20e53c248fa0c49db1070d3c3d30441fbc3675ef10c1c9a675b8897544d0177a421a876d52a27c2020409a2a2ee", 16);
	Unumber mod("ce9524c4b6e24e86b959ac6966f4f27657c890598834a9d554c16af4ac7d0876b65bc2559ec7ddc18c051544875fd26621466f4ef67d56139376e5039d1fe9", 16);
#elif KEYSIZE == 512
	/* key 512 */
	Unumber pri("", 16);
	Unumber pub("", 16);
	Unumber mod("", 16);
#elif KEYSIZE == 1024
	/* key 1024 */
	Unumber pri
("", 16);
	Unumber pub("", 16);
	Unumber mod("", 16);
#else
	/* key 2048 */
	Unumber pri("", 16);
	Unumber pub("", 16);
	Unumber mod("", 16);
#endif

	SecureInt::setKey(pri, pub, mod, ENCSIZE, ENCSIZE);

    int no_ads = 1 << 9;
    SecureInt distance = SecureInt::randomp2(ENCSIZE);
    SecureInt distance_squared = distance * distance;
    SecureInt user [2] = { SecureInt::randomp2(ENCSIZE), SecureInt::randomp2(ENCSIZE) };
    SecureInt ad_N [no_ads];
    SecureInt ad_E [no_ads];
    for ( size_t i=0; i<no_ads; i++ )
    {
        ad_N[i] = SecureInt::randomp2(ENCSIZE);
        ad_E[i] = SecureInt::randomp2(ENCSIZE);
    }
    SecureInt show_ad [no_ads];

	unsigned length = Unumber::HW_NUMWORDS;
	unsigned arr_distance_squared[length];
	SecureInt::exportArray(arr_distance_squared, length, distance_squared);
	unsigned arr_user[2][length];
	SecureInt::exportArray(arr_user[0], length, user[0]);
	SecureInt::exportArray(arr_user[1], length, user[1]);
	unsigned arr_ad_N[no_ads][length], arr_ad_E[no_ads][length];
	for ( size_t i=0; i<no_ads; i++ )
    {
		SecureInt::exportArray(arr_ad_N[i], length, ad_N[i]);
		SecureInt::exportArray(arr_ad_E[i], length, ad_E[i]);
    }
	unsigned arr_show_ad[no_ads][length];

    asm("l.debug");
	Unumber::mter_e0(arr_user[0]);
	Unumber::mter_e1(arr_user[1]);
	Unumber::mter_e2(arr_distance_squared);
	for ( size_t i=0; i<no_ads; i++ )
	{
		Unumber::mter_e3(arr_ad_N[i]);
		Unumber::mter_e4(arr_ad_E[i]);
		asm("le.ersub ze10,ze0,ze3");
		asm("le.emulu ze10,ze10,ze10");
		asm("le.ersub ze15,ze1,ze4");
		asm("le.emulu ze15,ze15,ze15");
		asm("le.eradd ze10,ze10,ze15");
		asm("le.eltu ze10,ze10,ze2");
		Unumber::mfer_e10(arr_show_ad[i]);
	}
    asm("l.debug");
	for ( size_t i=0; i<no_ads; i++ ) show_ad[i] = SecureInt::importArray(arr_show_ad[i], length);
    // Print out which advertised events are within the distance from the user. //
    // cout << "Advertisements relevant to the user's location:\n";
    // for (int i = 0; i < no_ads; i++)
    // {
    //     cout << show_ad[i].str() << " ";
    // }
	// cout << "\n";
}
