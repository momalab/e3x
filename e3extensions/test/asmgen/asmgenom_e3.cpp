...
SecureInt find(SecureInt & ref, SecureInt & sub, size_t rsz, size_t ssz)
{
    unsigned length = Unumber::HW_NUMWORDS;
    unsigned aref[length], asub[length], aresult[length];
    SecureInt::exportArray(aref, length, ref);
    SecureInt::exportArray(asub, length, sub);
	cout << "----------------------------------\n";
	asm("l.debug");
	Unumber::mter_e1(aref);
    Unumber::mter_e2(asub);
    ...
    */
    ...
    asm("l.debug");
    SecureInt pos = SecureInt::importArray(aresult, length);
    return pos;
}
...
("", 16);
    ...
    /**/
    string l = "CTGACTGACTGACTGA";
    string sref = "CTGACTGACTGACTGA";

    unsigned reads = s2i(l);
    unsigned refs = s2i(sref);

    size_t ssz = l.size();
    size_t rsz = sref.size();

    SecureInt ereads = SecureInt::encrypt(reads);
    SecureInt erefs = SecureInt::encrypt(erefs);

    SecureInt found = find(erefs, ereads, rsz, ssz);
