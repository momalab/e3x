...
SecureInt get(const SecureInt & n, const SecureInt & i)
{
    SecureInt s = i-i;
    s++;
    SecureInt mask = s;
    multiply_by_ull(31, s);
    multiply_by_ull(3, mask);
    s -= i+i;
    return mask & (n >> s);
}

SecureInt find(const SecureInt & ref, const SecureInt & sub, size_t rsz, size_t ssz)
{
    asm("l.debug");
    SecureInt zero = ref - ref;
    SecureInt one = zero;
    one++;
    SecureInt pos = one;
    ...
    if ( ssz <= rsz )
    {
        for ( auto k = rsz - ssz;; k-- )
        {
            ...
            {
                cout << "l\n";
                SecureInt refi = get(ref, ei);
                SecureInt subj = get(sub, ej);
                t = t == (refi == subj);
                ei++; ej++;
            }
...
("", 16);
    ...
    SecureInt erefs = SecureInt::encrypt(refs);

    SecureInt found = find(erefs, ereads, rsz, ssz);
    cout << " is found at: " << found.str() << endl;
}
